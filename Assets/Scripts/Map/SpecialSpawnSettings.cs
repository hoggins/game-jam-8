...
namespace Map
{
  ...
  public struct SpecialSpawnRange
  {
    ...
    [Tooltip("Distance range from the player this type may land at when the level first fills. For the Timer route, the seeded value from this range is the first route leg.")]
    public SpecialSpawnDistance initial;

    [Tooltip("Distance range for each successive runtime respawn, in order. For the Timer route these are world-unit leg lengths.")]

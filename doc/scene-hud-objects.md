...
Three are implemented today: `SceneHudElementId.BattleTimer` (the battle clock),
`SceneHudElementId.Arrow` (the compass pointing at the clock — see [The compass arrow](#the-compass-arrow))
and `SceneHudElementId.Hp` (the player's health — see [The health bar](#the-health-bar)). Design
intent for the rest — Ammo, Upgrade — is in [../Design/HudObjects.md](../Design/HudObjects.md).
...
[BattleHealthBar](../Assets/Scripts/Health/BattleHealthBar.cs) is the third implementation, and the
one to read if a new HUD object has to *write back* to the game state rather than only display it.
Its prefab is generated by
[BattleHealthPrefabBuilder](../Assets/Scripts/Health/Editor/BattleHealthPrefabBuilder.cs)
(*Tools ▸ Destruction ▸ Rebuild Battle Health Prefab*): 24 pixels in two rows of twelve, one
destructible group, at the timer's pixel size.

**The bar is the health, not a readout of it.** Each pixel is a twenty-fourth of the player's maximum,
and knocking one off costs him exactly that much. Smash all 24 and he dies — the object is a hazard he
can hurt himself on, not a pickup.

> **Damage only flows from pixels to health, never the other way.** Hitting the bar does *not* deal
> the player weapon damage: max health is `300000` against an attack power around `5`, so a self-
> inflicted hit would never remove a single pixel and the object would be inert. Instead the bar has
> its own durability (the `HealthBar` row in `BattleBalance.DestructibleMaxHealth`, spread across the
> 24 pixels); once enough damage has accumulated to pay for a pixel, the bar charges the player
> `MaxHealth / 24` and `CharacterService.HealthChanged` knocks the pixel off.

Everything funnels through one `Sync`, which derives the standing pixel count as
`ceil(current / max * 24)` and removes whatever the bar is over. That is what keeps the two directions
from double-counting: a duck bite and a hammer blow both end as a health change, and the bar only ever
mirrors what is left. Pixels come off in order via `DestructibleObject.FallOutPart`, so the bar empties
from one end instead of developing holes — `FallOutRandomPart` is the right call only when the parts
are interchangeable rubble.

> **`CharacterService.DestroyHealth()` is not this.** Despite the name it is the F10 invincibility
> cheat (`IsInvincible = true`); calling it here would make smashing the bar grant immortality. A
> consequence worth knowing: with that cheat on, `TakeDamage` is swallowed, health never moves, and
> the bar therefore cannot be destroyed either.

Two lifecycle rules differ from the other two objects. It is **never respawned or moved** —
`TimerRespawnService` skips it outright, so a fresh one would never hand back what the player spent
smashing the last one; it is placed once by `MapEnvironmentSpawner.SpawnInitialSpecials` and returns
only on the next battle.

And `Sync` does nothing until the battle has started, because the map is filled from
`LevelData.Start`, which can run before the battle deals the player his health — reading a health of
zero then would shatter the whole bar the moment it was placed.

> **That gate is a latch, not a live `IsBattleActive` check.** The battle ends the instant the player
> dies, and the health change that killed him is still being delivered as it does, so gating on
> "battle is active" leaves the bar standing over a corpse. It also silently stops mirroring anything
> that hurts the player after a timeout defeat. `_isLive` latches on at the first `BattleStarted` (or
> at `OnEnable` if the battle is already running, since the event will not fire again) and never
> clears.
